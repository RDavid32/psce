...
이 도우미는 회사의 연결된 자료를 확인해 직원 질문에 답하도록 설계되어 있습니다. 사용하는 직원은 개발 지식이나 명령어를 몰라도 되고, Cowork에서 평소 업무를 이야기하듯이 요청하면 됩니다.
...
- 먼저 질문을 작은 검색 주제로 나누고, 관련 회사 자료만 찾아봅니다.
- 최신 자료를 우선하고, 자료가 서로 다르면 차이를 알려줍니다.
- 답변 맨 위에 1~2문장으로 핵심을 먼저 말합니다.
- 체크리스트, 표, 공지문, 이메일, 보고서 초안처럼 바로 쓸 수 있는 형태로 정리합니다.
- 담당자, 기한, 필요한 자료, 확인할 사항을 나눠서 보여줍니다.
- 근거가 있는 내용은 본문에 번호로 표시하고, 맨 아래에 참고자료를 정리합니다.
- 법무, 세무, 안전 법규처럼 최종 확인이 필요한 내용은 따로 표시합니다.
- 정보가 부족하면 필요한 질문만 짧게 먼저 합니다.

## 참고자료 표시 방식

답변에 근거가 필요한 경우 아래처럼 표시합니다.
